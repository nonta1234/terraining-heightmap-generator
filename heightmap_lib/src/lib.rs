...
pub struct Heightmap {
    ...
    pub pointer_to_oceanmap: *const f32,
...
impl Heightmap {
    ...
    pub fn new() -> Self {
        Self {
            ...
            pointer_to_oceanmap: std::ptr::null(),
    ...
    pub fn allocate_map_buffer(&mut self, length: usize) {
        ...
        self.oceanmap = Vec::with_capacity(length);
        unsafe { self.oceanmap.set_len(length) }
        self.pointer_to_oceanmap = self.oceanmap.as_ptr();
    ...
    pub fn combine_heightmaps(&mut self, water_depth: usize, stream_depth: usize) {
        for i in 0..(self.heightmap.len()) {
            let h = if self.heightmap[i] == 0.0 { self.oceanmap[i] } else { self.heightmap[i] };
            let water_effect = ((1.0 - self.watermap[i]) * (water_depth as f32)).max(
                (1.0 - self.waterwaymap[i]) * (stream_depth as f32)
            );
            self.result[i] = h - water_effect;
        }
    }
